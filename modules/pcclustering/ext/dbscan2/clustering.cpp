...
namespace dbscan2 {
    ...
    public:
        ...
        double getDis(const Point & ot) {
            ...
            for (int i = 0; i < data.size(); ++i) {
                total += pow(data[i] - ot.data[i], 2.0);
            }
            return pow(total, 1.0 / data.size());
    ...
    public:
        ...
        inline void checkNearPoints() {
            for (int i = 0; i < size; i++) {
                for (int j = 0; j < size; j++) {
                    if (i == j) continue;
                    const double d = points[i].getDis(points[j]);
                    if (d <= eps) {
                        points[i].ptsCnt++;
        ...
        inline bool isCoreObject(int idx) {
